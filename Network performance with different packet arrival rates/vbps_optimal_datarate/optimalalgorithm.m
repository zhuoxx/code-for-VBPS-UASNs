...
for iNA = 1:NA
    ...
    for iarrival = 2:vutotaltimearrival(iNA,ivutrafficload) %Each data packets arrival time
        ...
        if vuAUVtimebefore(iNA,iarrival)>0 && vuAUVtimebefore(iNA,iarrival)<len_xyz(iNA)-1
            ...
            %Calculate whether the static noeds is in the communication range of AUV
            ...
            if vuknown_nodesincount(iNA,iarrival)>= 1
                for i = 1:vuknown_nodesincount(iNA,iarrival)
                    ...
                    vureceiveauvrun_timecase1 = vutimeslot_number(iNA,iarrival)*n*t_data+vutimeslot_numberplus(iNA,iarrival)+vuAUVrun_distancenode(iNA,iarrival,i)/vs;%AUV arrival time
                    if abs(vureceiveauvrun_timecase1+t_data-(vutimeslot_number(iNA,iarrival)+1)*n*t_data)<t_data && ismember(vuAUVrun_knownnodesin(iNA,iarrival,i), timeslot(vutimeslot_number(iNA,iarrival)+1).nodeset) == 1%The data packet is sent by AUV at the current slot, but may be receieved by static nodes at the next time slot
                        ...
                        if vuAUVtimebefore(iNA,iarrival)>0 && vuAUVtimebefore(iNA,iarrival)<len_xyz(iNA)-1
                            ...
                            if vuknown_nodesincount(iNA,iarrival)>= 1
                                ...
                                for i = 1:vuknown_nodesincount(iNA,iarrival)
                                    ...
                                    % Case 3: Receiving-Sending Collisions at  the
                                    % interference  nodes
                                ...
                                break;%Until find this time slot
...
%  Collisions Calculation among multiple AUVs
            ...
            for i = 1:vuknown_nodesincount(iNA,iarrival)
                if vuqueuesendtime(iNA,iarrival,i)>0
                    ...
                    if vuqueuesendtime(iNA,iarrival,i)>0 && vuqueuesendtime(iNA,iarrival,i)<len_xyz(iNA)*timeplus-timestart-timeend-1
                        vuAx_packetsendtime(iNA,iarrival,i) = Ax(iNA,vuqueuesendtimebefore(iNA,iarrival,i))+(Ax(iNA,vuqueuesendtimebefore(iNA,iarrival,i)+1)-Ax(iNA,vuqueuesendtimebefore(iNA,iarrival,i)))/timeplus*vuqueuesendtimeplus(iNA,iarrival,i);
        ...
        if vuAUVtimebefore(iNA,iarrival)>0 && vuAUVtimebefore(iNA,iarrival)<len_xyz(iNA)
            ...
            for i = 1:vuknown_nodesincount(iNA,iarrival)
                if vuqueuesendtime(iNA,iarrival,i)>0 && vuqueuesendtime(iNA,iarrival,i)<len_xyz(iNA)*timeplus-timestart-timeend-1
                    ...
                    elseif vuAUVrun_knownnodesin(iNA,iarrival,i)>N0+N1     %The sink node
                        vutimeslot_temp = floor(vuqueuesendtime(iNA,iarrival,i)/(n*t_data));
                        vuround_temp = floor(vutimeslot_temp/path(vuAUVrun_knownnodesin(iNA,iarrival,i)).count);
                        vuroundplus_temp = mod(vutimeslot_temp,path(vuAUVrun_knownnodesin(iNA,iarrival,i)).count);
...
for iNA = 1:NA
    for iarrival = 1:vutotaltimearrival(iNA,ivutrafficload)
        if vuAUVtimebefore(iNA,iarrival)>0 && vuAUVtimebefore(iNA,iarrival)<len_xyz(iNA)-1 &&vuvoi_choosenode(iNA,iarrival)~= 0
            ...
            if  vuvoiauvcollisioncount_choose(iNA,iarrival)~= 0
