%%%%%%%%%%%%%%%%%%%%%% Main file for VBPS-optimal with different datarate
clc;clear;close all;
...
path1 = 'C:\Users\zxx\Desktop\upload\Network performance with different packet arrival rates\vbps_optimal_datarate\networkdata'; %networkdata file path
path2 = 'C:\Users\zxx\Desktop\upload\Network performance with different packet arrival rates\vbps_optimal_datarate\plot_data'; %plotdata file path
...
for iteration=1:iteration_total
    ...
    for ivutrafficload=1:length(vutrafficload)  %pkt/s 
         ...
         %delaycomputing function for vu
            ...
            [vuvoi_collision,vuarrivaltime,vuAUVrun_in,vuAUVrun_knownnodes,vuAUVrun_knownnodesin,vuAUVtimebefore,vuAUVtimeplus,vutimeslot_number,vutimeslot_numberplus,vudiscardnodecount,vuknown_nodescount,vuknown_nodesincount,vuqueuearrivaltime,vuqueuecount,vuqueueindex,vuqueueout,vuqueuesendtime,vusendauvrun_time,vusendauvrun_timecase1,vusendauvrun_timecase2,vusendauvrun_timecase3,vusendauvrun_timecase4,vusenddelay,vutime_interval,vutotaldelay,vuunknown_nodescount,vuunknown_nodesincount,vuAUVrun_distancenode,vucollisioncount,vutotaltimearrival,vuvoi_choose,vuvoi_chooseindex,vuvoi_choosenode,vuvoitotaldelay_choose,vuvoi,vuauvcollisioncount,vuvoisenddelay_choose,vuvoiauvcollisioncount_choose] = optimalalgorithm(Nx,Ny,Nz,node,path,timeslot,len_xyz,dis_table,Ax,Ay,Az,AUV_in,vulambda,NA,N,timeplus,n,t_data,N0,N1,N2,vs,vutrafficload,ivutrafficload,iteration,betau,vu0,alphau,Tu,L_packet,timestart,timeend);

         for ivmtrafficload=1:length(vmtrafficload)  
            vmlambda=vmtrafficload(ivmtrafficload); %pkt/s 
            [vmvoi_collision,vmarrivaltime,vmAUVrun_in,vmAUVrun_knownnodes,vmAUVrun_knownnodesin,vmAUVtimebefore,vmAUVtimeplus,vmtimeslot_number,vmtimeslot_numberplus,vmdiscardnodecount,vmknown_nodescount,vmknown_nodesincount,vmqueuearrivaltime,vmqueuecount,vmqueueindex,vmqueueout,vmqueuesendtime,vmsendauvrun_time,vmsendauvrun_timecase1,vmsendauvrun_timecase2,vmsendauvrun_timecase3,vmsendauvrun_timecase4,vmsenddelay,vmtime_interval,vmtotaldelay,vmunknown_nodescount,vmunknown_nodesincount,vmAUVrun_distancenode,vmcollisioncount,vmtotaltimearrival,vmvoi_choose,vmvoi_chooseindex,vmvoi_choosenode,vmvoitotaldelay_choose,vmvoi,vmauvcollisioncount,vmvoisenddelay_choose,vmvoiauvcollisioncount_choose] = optimalalgorithm(Nx,Ny,Nz,node,path,timeslot,len_xyz,dis_table,Ax,Ay,Az,AUV_in,vmlambda,NA,N,timeplus,n,t_data,N0,N1,N2,vs,vmtrafficload,ivmtrafficload,iteration,betam,vm0,alpham,Tm,L_packet,timestart,timeend);
...
%              
                ...
                if vuvoiarrivaltime(iNA_plot,iteration,ivutrafficload,ivmtrafficload)~=0
                    ...
                    vuvoidiscardnodecount_plot(iNA_plot,iteration,ivutrafficload,ivmtrafficload)=sum(vuvoidiscardnodecount(iNA_plot,:))/vutotaltimearrival(iNA_plot,ivutrafficload); %周围没有节点，也没法传输 rate
                     vuvoiauvcollisioncount_plot(iNA_plot,iteration,ivutrafficload,ivmtrafficload)=sum(vuvoiauvcollisioncount_choose(iNA_plot,:))/vutotaltimearrival(iNA_plot,ivutrafficload);%由于不是globalmap导致冲突 rate
                    
                    vuvoithroughput_plot(iNA_plot,iteration,ivutrafficload,ivmtrafficload)=(vuvoiarrivaltime(iNA_plot,iteration,ivutrafficload,ivmtrafficload)-sum(vuvoiauvcollisioncount_choose(iNA_plot,:)))*t_data/(len_xyz(iNA_plot)*timeplus-timestart-timeend);%吞吐量
          end
            end
...
%   
                ...
                if vmvoiarrivaltime(iNA_plot,iteration,ivutrafficload,ivmtrafficload)~=0
                    ...
                    vmvoidiscardnodecount_plot(iNA_plot,iteration,ivutrafficload,ivmtrafficload)=sum(vmvoidiscardnodecount(iNA_plot,:))/vmtotaltimearrival(iNA_plot,ivmtrafficload); %周围没有节点，也没法传输 rate zxx1209
                     vmvoiauvcollisioncount_plot(iNA_plot,iteration,ivutrafficload,ivmtrafficload)=sum(vmvoiauvcollisioncount_choose(iNA_plot,:))/vmtotaltimearrival(iNA_plot,ivmtrafficload);%由于不是globalmap导致冲突 rate zxx1209
                    vmvoithroughput_plot(iNA_plot,iteration,ivutrafficload,ivmtrafficload)=(vmvoiarrivaltime(iNA_plot,iteration,ivutrafficload,ivmtrafficload)-sum(vmvoiauvcollisioncount_choose(iNA_plot,:)))*t_data/(len_xyz(iNA_plot)*timeplus-timestart-timeend);%吞吐量
...
%    %%save data
% 
% save(fullfile(path2,'20211220vbpsdatarate_vuvoi_choose_global'),'vuvoi_choose');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoi_collision_global'),'vuvoi_collision');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoi_plot_global'),'vuvoi_plot');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoi_collision_plot_global'),'vuvoi_collision_plot');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoi_plot_sum_global'),'vuvoi_plot_sum');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoi_collision_plot_sum_global'),'vuvoi_collision_plot_sum');
% 
% save(fullfile(path2,'20211220vbpsdatarate_vuvoitotaldelay_choose_global'),'vuvoitotaldelay_choose');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoitotaldelay_plot_global'),'vuvoitotaldelay_plot');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoisenddelay_choose_global'),'vuvoisenddelay_choose');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoisenddelay_plot_global'),'vuvoisenddelay_plot');
% 
% save(fullfile(path2,'20211220vbpsdatarate_vuvoidiscardnodecount_plot_global'),'vuvoidiscardnodecount_plot');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoiauvcollisioncount_plot_global'),'vuvoiauvcollisioncount_plot');
% 
% save(fullfile(path2,'20211220vbpsdatarate_vuvoithroughput_plot_global'),'vuvoithroughput_plot');
% 
% save(fullfile(path2,'20211220vbpsdatarate_vuvoiarrivaltime_global'),'vuvoiarrivaltime');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoitotaldelay_plot_sum_global'),'vuvoitotaldelay_plot_sum');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoisenddelay_plot_sum_global'),'vuvoisenddelay_plot_sum');
% 
% save(fullfile(path2,'20211220vbpsdatarate_vmvoi_choose_global'),'vmvoi_choose');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoi_collision_global'),'vmvoi_collision');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoi_plot_global'),'vmvoi_plot');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoi_collision_plot_global'),'vmvoi_collision_plot');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoi_plot_sum_global'),'vmvoi_plot_sum');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoi_collision_plot_sum_global'),'vmvoi_collision_plot_sum');
% 
% save(fullfile(path2,'20211220vbpsdatarate_vmvoitotaldelay_choose_global'),'vmvoitotaldelay_choose');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoitotaldelay_plot_global'),'vmvoitotaldelay_plot');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoisenddelay_choose_global'),'vmvoisenddelay_choose');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoisenddelay_plot_global'),'vmvoisenddelay_plot');
% 
% save(fullfile(path2,'20211220vbpsdatarate_vmvoidiscardnodecount_plot_global'),'vmvoidiscardnodecount_plot');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoicollisioncount_plot_global'),'vmvoicollisioncount_plot');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoiauvcollisioncount_plot_global'),'vmvoiauvcollisioncount_plot');
% 
% save(fullfile(path2,'20211220vbpsdatarate_vmvoithroughput_plot_global'),'vmvoithroughput_plot');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoitotalthroughput_plot_global'),'vmvoitotalthroughput_plot');
% 
% save(fullfile(path2,'20211220vbpsdatarate_vmvoiarrivaltime_global'),'vmvoiarrivaltime');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoicollisioncount_plot_global'),'vuvoicollisioncount_plot');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoitotalthroughput_plot_global'),'vuvoitotalthroughput_plot');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoitotaldelay_plot_sum_global'),'vmvoitotaldelay_plot_sum');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoisenddelay_plot_sum_global'),'vmvoisenddelay_plot_sum');
 iteration
end
...
for iNA_plot=1:NA
 ...
 %% Save VoI and network delay
% save(fullfile(path2,'20211220vbpsdatarate_vuvoi_vbps_average_global'),'vuvoi_vbps_average');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoi_collision_vbps_average_global'),'vuvoi_collision_vbps_average');
% 
% save(fullfile(path2,'20211220vbpsdatarate_vuvoitotaldelay_vbps_average_global'),'vuvoitotaldelay_vbps_average');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoisenddelay_vbps_average_global'),'vuvoisenddelay_vbps_average');
% 
% save(fullfile(path2,'20211220vbpsdatarate_vuvoidiscardnodecount_vbps_average_global'),'vuvoidiscardnodecount_vbps_average');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoiauvcollisioncount_vbps_average_global'),'vuvoiauvcollisioncount_vbps_average');
% 
% save(fullfile(path2,'20211220vbpsdatarate_vuvoithroughput_vbps_average_global'),'vuvoithroughput_vbps_average');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoitotalthroughput_vbps_average_global'),'vuvoitotalthroughput_vbps_average');


% save(fullfile(path2,'20211220vbpsdatarate_vmvoi_vbps_average_global'),'vmvoi_vbps_average');
% save(fullfile(path2,'20211220vbpsdatarate_vuvoicollisioncount_vbps_average_global'),'vuvoicollisioncount_vbps_average');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoi_collision_vbps_average_global'),'vmvoi_collision_vbps_average');
% 
% save(fullfile(path2,'20211220vbpsdatarate_vmvoitotaldelay_vbps_average_global'),'vmvoitotaldelay_vbps_average');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoisenddelay_vbps_average_global'),'vmvoisenddelay_vbps_average');
% 
% save(fullfile(path2,'20211220vbpsdatarate_vmvoidiscardnodecount_vbps_average_global'),'vmvoidiscardnodecount_vbps_average');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoicollisioncount_vbps_average_global'),'vmvoicollisioncount_vbps_average');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoiauvcollisioncount_vbps_average_global'),'vmvoiauvcollisioncount_vbps_average');
% 
% save(fullfile(path2,'20211220vbpsdatarate_vmvoithroughput_vbps_average_global'),'vmvoithroughput_vbps_average');
% save(fullfile(path2,'20211220vbpsdatarate_vmvoitotalthroughput_vbps_average_global'),'vmvoitotalthroughput_vbps_average');
...
for ivutrafficload=length(vutrafficload)%1:length(vmtrafficload)
    for i=1:length(vmtrafficload)
        A(i)=sum(vmvoiauvcollisioncount_vbps_average(:,ivutrafficload,i));
    end
    plot(vmtrafficload(2:end),A(2:end).','*-b','LineWidth',1.8,'MarkerSize',8,'color',clr(1,:));
    hold on
end
