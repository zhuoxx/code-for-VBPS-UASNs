...
% listened static nodes and topology. Then AUV calculate the collision 
% constraints and then select the sending time of data packets and the next-hop node based on VoI maximization problem. 
                        ...
                        if vuAUVtimebefore(iNA,iarrival)>0 && vuAUVtimebefore(iNA,iarrival)<len_xyz(iNA)-1
                            ...
                            if vuknown_nodesincount(iNA,iarrival)>=1
                                ...
                                for i = 1:vuknown_nodesincount(iNA,iarrival)
                                    ...
                                    % Case 3: Receiving-Sending Collisions
                                    % at  the interference  nodes
    ...
    for iarrival = 1:vutotaltimearrival(iNA,ivutrafficload)  
        if vuAUVtimebefore(iNA,iarrival)>0 && vuAUVtimebefore(iNA,iarrival)<len_xyz(iNA)-1 &&vuvoi_chooseindex(iNA,iarrival)~=0
            for jn = 1:vuunknown_nodesincount(iNA,iarrival)
                ...
                if abs(vuunreceiveauvrun_timecase1+t_data-(vutimeslot_number(iNA,iarrival)+1)*n*t_data)<t_data && ismember(vuAUVrun_unknownnodesin(iNA,iarrival,jn), timeslot(vutimeslot_number(iNA,iarrival)+1).nodeset)==1 
