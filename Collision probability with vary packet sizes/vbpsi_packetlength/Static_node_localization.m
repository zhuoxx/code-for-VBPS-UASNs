function [AUV_listen,AUV_detected,AUV_in] =  Static_node_localization(Ax, Ay, Az,Ts, Nx, Ny, Nz,n,t_data,path1,N0,N1,N2,NA,N,Round,totaltimeslot,timeplus,path,node,len_xyz,dc,iL_packet)
...
% static nodes.
% save(fullfile(path1,strcat('AUV_listen',num2str(iL_packet))),'AUV_listen');
% save(fullfile(path1,strcat('AUV_detected',num2str(iL_packet))),'AUV_detected');
% save(fullfile(path1,strcat('AUV_in',num2str(iL_packet))),'AUV_in');

% path1='D:\software\Matlab\install\work\LAB\zxxpaper\2021auv';
AUV_listen = cell2mat(struct2cell(load(fullfile(path1,strcat('AUV_listen',num2str(iL_packet))))));
AUV_detected = cell2mat(struct2cell(load(fullfile(path1,strcat('AUV_detected',num2str(iL_packet))))));
AUV_in = cell2mat(struct2cell(load(fullfile(path1,strcat('AUV_in',num2str(iL_packet))))));
